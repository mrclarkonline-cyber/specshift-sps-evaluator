...
OUT_PATH = OUT_DIR / "phase2_final3_source_access_repair_records.jsonl"
...
def curl_check(url: str) -> dict:
    cmd = [
        ...
        "--connect-timeout", "12",
        "--max-time", "30",
        "-A", "Mozilla/5.0 (Macintosh; Intel Mac OS X) SpecShift-Labs-source-access-validation/0.1",
        "-H", "Accept: text/html,application/xhtml+xml,application/xml,application/json,*/*",
        "-sS",
        "-o", "/tmp/specshift_final3_source_access.tmp",
...
def check_target(target: dict) -> dict:
    ...
    return {
        ...
        "batch": "final3_source_access_repair",
...
print(f"Final-3 source-access passed: {passed}/{len(records)}")
for rec in records:
